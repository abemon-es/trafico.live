...
# R2: configure rclone remote named "r2" with bucket "trafico-backups" before use.
#     rclone config → new remote → type=s3 → provider=Cloudflare → access_key/secret from R2 dashboard.
...
# ---------------------------------------------------------------------------
# 4. Upload to Cloudflare R2 via rclone (optional — warn if unconfigured)
# ---------------------------------------------------------------------------
R2_OK=0
if command -v rclone >/dev/null 2>&1; then
  if rclone listremotes 2>/dev/null | grep -q "^${R2_REMOTE}:"; then
    DUMP_BASENAME="$(basename "$DUMP_FILE")"
    log "Uploading $DUMP_BASENAME to ${R2_REMOTE}:${R2_BUCKET}/${R2_PREFIX}/ ..."
    rclone copy "$DUMP_FILE" "${R2_REMOTE}:${R2_BUCKET}/${R2_PREFIX}/" \
      --progress \
      --stats-one-line \
      && R2_OK=1 \
      && log "R2 upload OK" \
      || log "WARN: R2 upload failed — backup remains local only"
    ...
    if [ "$DAY_OF_WEEK" = "7" ] && [ -f "${BACKUP_DIR}/weekly-${TIMESTAMP}.dump" ]; then
      rclone copy "${BACKUP_DIR}/weekly-${TIMESTAMP}.dump" \
        "${R2_REMOTE}:${R2_BUCKET}/weekly/" \
        --stats-one-line \
        || log "WARN: R2 weekly upload failed"
    fi
    if [ "$DAY_OF_MONTH" = "1" ] && [ -f "${BACKUP_DIR}/monthly-${TIMESTAMP}.dump" ]; then
      rclone copy "${BACKUP_DIR}/monthly-${TIMESTAMP}.dump" \
        "${R2_REMOTE}:${R2_BUCKET}/monthly/" \
        --stats-one-line \
        || log "WARN: R2 monthly upload failed"
    fi
  else
    log "WARN: rclone remote '${R2_REMOTE}' not configured — skipping R2 upload"
  fi
else
  log "WARN: rclone not installed — skipping R2 upload"
fi
...
if command -v curl >/dev/null 2>&1; then
  ...
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    -d "{\"status\":$([ "$BACKUP_STATUS" = "1" ] && echo '"ok"' || echo '"fail"'),\"dump\":\"$(basename "$DUMP_FILE")\",\"r2_uploaded\":${R2_OK}}" \
    2>/dev/null || echo "000")
  if [[ "$HTTP_CODE" =~ ^2 ]]; then
