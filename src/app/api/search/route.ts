...
const SEARCH_CONFIGS: CollectionSearchConfig[] = [
  ...
  {
    collection: "voyages",
    queryBy: "vesselName,departurePort,arrivalPort,vesselFlag",
    queryByWeights: "3,2,2,1",
    category: "Viajes marítimos",
    icon: "Ship",
    mapHit: (doc) => ({
      title: (doc.vesselName as string) || `MMSI ${doc.mmsi as number}`,
      subtitle: [doc.departurePort, doc.arrivalPort].filter(Boolean).join(" → ") || (doc.status as string) || null,
      href: `/maritimo?mmsi=${doc.mmsi as number}`,
      category: "Viajes marítimos",
      icon: "Ship",
    }),
  },
  {
    collection: "port_calls",
    queryBy: "portName,portCode",
    queryByWeights: "3,2",
    category: "Escalas portuarias",
    icon: "Anchor",
    mapHit: (doc) => ({
      title: (doc.portName as string) || (doc.portCode as string) || `Puerto MMSI ${doc.mmsi as number}`,
      subtitle: (doc.ongoing as boolean) ? "En puerto" : `MMSI ${doc.mmsi as number}`,
      href: `/maritimo?port=${encodeURIComponent((doc.portCode as string) || (doc.portName as string) || "")}`,
      category: "Escalas portuarias",
      icon: "Anchor",
    }),
  },
