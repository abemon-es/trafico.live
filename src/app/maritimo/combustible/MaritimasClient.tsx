...
> = {
  "Mediterráneo": {
    label: "Mediterráneo",
    color: "bg-[var(--color-tl-sea-50)] dark:bg-[color-mix(in_oklch,var(--color-tl-sea-900)_20%,transparent)]",
    borderColor: "border-[var(--color-tl-sea-200)]",
    icon: "🌊",
...
const FUEL_TYPES = [
  ...
  {
    name: "Gasóleo B",
    badge: "bg-[var(--color-tl-sea-100)] text-[var(--color-tl-sea-800)]",
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <nav aria-label="Migas de pan" className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 mb-6">
        ...
        <Link href="/maritimo" className="hover:text-gray-700 dark:text-gray-300 transition-colors">Marítimo</Link>
        <ChevronRight className="w-3.5 h-3.5 text-gray-400" />
        <span className="text-gray-900 dark:text-gray-100 font-medium">Combustible</span>
      </nav>

      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 flex items-center gap-3 mb-3">
          <Anchor className="w-8 h-8 text-[var(--color-tl-sea-600)] dark:text-[var(--color-tl-sea-400)]" />
      ...
      <div className="bg-[var(--color-tl-sea-50)] dark:bg-[color-mix(in_oklch,var(--color-tl-sea-900)_20%,transparent)] rounded-lg border border-[var(--color-tl-sea-200)] dark:border-[var(--color-tl-sea-700)] p-6 mb-8">
        <div className="flex items-start gap-4">
          <Ship className="w-8 h-8 text-[var(--color-tl-sea-600)] dark:text-[var(--color-tl-sea-400)] flex-shrink-0 mt-0.5" />
          <div>
            <h2 className="text-lg font-semibold text-[var(--color-tl-sea-900)] mb-2">
              ¿Quién necesita combustible en puerto?
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm text-[var(--color-tl-sea-800)] dark:text-[var(--color-tl-sea-200)]">
              <div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-6 mb-8">
          ...
          <div className="space-y-2">
            {cheapestDiesel.map((s, idx) => (
              <Link
                key={s.id}
                href={`/maritimo/combustible/${s.id}`}
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
              activeRegion === null
                ? "bg-[var(--color-tl-sea-600)] text-white"
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4 mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-[var(--color-tl-sea-500)] dark:ring-[var(--color-tl-sea-400)] focus:border-transparent"
            />
          </div>
          <button
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
            <div className="max-w-xs">
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-[var(--color-tl-sea-500)] dark:ring-[var(--color-tl-sea-400)]"
              >
        ...
        {province && (
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="flex items-center gap-1 px-2 py-1 bg-[var(--color-tl-sea-100)] dark:bg-[color-mix(in_oklch,var(--color-tl-sea-900)_30%,transparent)] text-[var(--color-tl-sea-700)] dark:text-[var(--color-tl-sea-300)] rounded-full text-sm">
              {PROVINCE_NAMES[province]}
              <button onClick={() => setProvince("")} className="hover:text-[var(--color-tl-sea-900)]">
                <X className="w-4 h-4" />
              </button>
            </span>
          </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-12">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--color-tl-sea-600)]" />
        </div>
      ) : error ? (
        ...
        <>
          ...
          {(search || activeRegion ? [null] : regionOrder).map((regionKey) => {
            ...
            // Keep the variable to avoid lint errors even though it's only used for the shape
            void regionsToShow;
      ...
      <div className="mt-4 border-t border-gray-200 dark:border-gray-800 pt-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {[
            { href: "/maritimo/meteorologia", label: "Meteorología marítima" },
            ...
            <Link
              key={link.href}
              href={link.href}
              className="flex items-center gap-2 p-3 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 hover:border-[var(--color-tl-sea-300)] hover:shadow-sm transition-all group text-sm"
            >
              <Anchor className="w-4 h-4 text-[var(--color-tl-sea-500)] flex-shrink-0" />
              <span className="text-gray-700 dark:text-gray-300 group-hover:text-[var(--color-tl-sea-700)] dark:group-hover:text-[var(--color-tl-sea-300)] transition-colors line-clamp-1">
                {link.label}
              </span>
            </Link>
          ))}
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0">
          ...
          {station.port && (
            <p className="text-sm text-[var(--color-tl-sea-600)] dark:text-[var(--color-tl-sea-400)] font-medium truncate">Puerto: {station.port}</p>
          )}
        ...
        {station.is24h && (
          <span className="flex items-center gap-1 px-2 py-0.5 bg-[var(--color-tl-sea-100)] dark:bg-[color-mix(in_oklch,var(--color-tl-sea-900)_30%,transparent)] text-[var(--color-tl-sea-700)] dark:text-[var(--color-tl-sea-300)] text-xs rounded-full ml-2 flex-shrink-0">
      ...
      {station.priceGasoleoB != null && (
        <div className="mt-2 bg-[var(--color-tl-sea-50)] dark:bg-[color-mix(in_oklch,var(--color-tl-sea-900)_20%,transparent)] rounded p-2">
          <div className="text-xs text-[var(--color-tl-sea-600)]">Gasóleo B (pesca)</div>
          <div className="font-bold text-[var(--color-tl-sea-700)]">{formatP(station.priceGasoleoB)}</div>
        </div>
      )}
