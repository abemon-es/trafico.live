...
function getZBEStatus(zone: ZBEZone): { label: string; color: string } {
  ...
  if (from > now) return { label: "Planificada", color: "bg-tl-amber-100 text-tl-amber-700 dark:text-tl-amber-300" };
  if (until && until < now) return { label: "Expirada", color: "bg-gray-100 dark:bg-gray-900 text-gray-700 dark:text-gray-300" };
  return { label: "Activa", color: "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400" };
}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="space-y-1 text-sm">
                          ...
                          {zone.activeAllYear && (
                            <p className="text-xs text-red-600 dark:text-red-400 mt-1">Activa todo el año</p>
                          )}
                        </div>

                        {restrictionEntries.length > 0 && (
                          <div className="mt-3">
                            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Restricciones:</p>
                            <div className="flex flex-wrap gap-1">
                              {restrictionEntries.map(([key, value]) => (
                                <span
                                  key={key}
                                  className="text-xs bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 px-2 py-0.5 rounded"
                                >
                                  {value || key}
                                </span>
                              ))}
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4 text-center">
                <Ruler className="w-6 h-6 text-tl-600 dark:text-tl-400 mx-auto mb-2" />
                <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Altura</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Límites verticales</p>
              </div>
              <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4 text-center">
                <Scale className="w-6 h-6 text-tl-amber-600 dark:text-tl-amber-400 mx-auto mb-2" />
              ...
              <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800 p-4 text-center">
                <Truck className="w-6 h-6 text-red-600 dark:text-red-400 mx-auto mb-2" />
                <p className="text-sm font-medium text-gray-900 dark:text-gray-100">Anchura</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">Límites laterales</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {ROAD_RESTRICTIONS.map((restriction) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h3 className="font-medium text-gray-900 dark:text-gray-100">{restriction.name}</h3>
                      <p className="text-sm text-tl-600 dark:text-tl-400">{restriction.road}</p>
                    </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-950 rounded-lg p-3 mt-2">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                      {restriction.restriction}
                    </p>
                    {restriction.notes && (
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{restriction.notes}</p>
                    )}
                  </div>
                </div>
              ))}
