...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${BASE_URL}/precio-diesel-hoy`,
    },
    openGraph: {
...
export default async function PrecioDieselHoyPage() {
  ...
  const sortedByPrice = [...peninsulaProvinces].sort((a, b) => {
    const pa = toNum(a.avgGasoleoA) ?? Infinity;
    const pb = toNum(b.avgGasoleoA) ?? Infinity;
    return pa - pb;
  });
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {peninsulaProvinces.length > 0 && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 mb-8 shadow-sm">
            ...
            {/* Helper to render a province table section */}
                        ...
                        {rows.map((p) => {
                          ...
                          return (
                            <tr key={p.scope} className="border-b border-gray-50 hover:bg-gray-50 dark:bg-gray-950 transition-colors">
                              ...
                              <td className="py-3 px-2 text-right text-gray-600 dark:text-gray-400 hidden sm:table-cell font-data">{formatPrice(p.minGasoleoA)}</td>
                              <td className="py-3 px-2 text-right text-gray-600 dark:text-gray-400 hidden sm:table-cell font-data">{formatPrice(p.maxGasoleoA)}</td>
