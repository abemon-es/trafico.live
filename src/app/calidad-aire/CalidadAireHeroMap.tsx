  ...
  () => import("@/components/map/TraficoMap").then((m) => m.TraficoMap),
  {
    ...
    loading: () => (
      <div className="w-full h-[500px] md:h-[600px] bg-tl-50 dark:bg-slate-900 animate-pulse rounded-2xl flex items-center justify-center">
        <Wind className="w-12 h-12 text-tl-300" />
      </div>
    ),
  }
);

export function CalidadAireHeroMap() {
  return (
    <TraficoMap
      initialLayers={defaultLensLayers("aire")}
      controls={{ lensBar: "aire", layerPanel: true, legend: true, themeToggle: true, fullscreen: false }}
